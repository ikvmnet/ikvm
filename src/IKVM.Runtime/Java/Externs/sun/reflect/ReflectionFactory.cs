...
namespace IKVM.Java.Externs.sun.reflect
{
    ...
    static class ReflectionFactory
    {
        ...
        sealed class SerializationConstructorAccessorImpl : global::sun.reflect.ConstructorAccessor
        {
            ...
            {
#if NETFRAMEWORK
                var obj = FormatterServices.GetUninitializedObject(type.AsReflection());
#else
                var obj = RuntimeHelpers.GetUninitializedObject(type.AsReflection());
#endif
                if (mw != null)
        ...
        sealed class FastMethodAccessorImpl : global::sun.reflect.MethodAccessor
        {
            ...
            internal unsafe FastMethodAccessorImpl(RuntimeJavaMethod mw)
            {
                try
                {
                    ...
                    var dm = DynamicMethodUtil.Create($"__<FastMethodAccessor>__{mw.DeclaringType.Name.Replace(".", "_")}__{mw.Name}", mw.DeclaringType.TypeAsBaseType.AsReflection(), np, typeof(object), new[] { typeof(object), typeof(object[]), typeof(global::ikvm.@internal.CallerID) });
        ...
        sealed class FastConstructorAccessorImpl : global::sun.reflect.ConstructorAccessor
        {
            ...
            internal FastConstructorAccessorImpl(global::java.lang.reflect.Constructor constructor)
            {
                try
                {
                    ...
                    mw.ResolveMethod();
                    var np = !mw.IsPublic || !mw.DeclaringType.IsPublic;
                    var dm = DynamicMethodUtil.Create($"__<FastConstructorAccessor>__{mw.DeclaringType.Name.Replace(".", "_")}__{mw.Name}", mw.DeclaringType.TypeAsTBD.AsReflection(), np, typeof(object), new[] { typeof(object[]) });
                    var il = JVM.Context.CodeEmitterFactory.Create(dm);
        ...
        sealed class FastSerializationConstructorAccessorImpl : global::sun.reflect.ConstructorAccessor
        {
            ...
            internal FastSerializationConstructorAccessorImpl(global::java.lang.reflect.Constructor constructorToCall, global::java.lang.Class classToInstantiate)
            {
                ...
                var dm = DynamicMethodUtil.Create("__<SerializationCtor>", constructor.DeclaringType.TypeAsBaseType.AsReflection(), true, typeof(object), null);
                var il = JVM.Context.CodeEmitterFactory.Create(dm);
        ...
        sealed class ActivatorConstructorAccessor : global::sun.reflect.ConstructorAccessor
        {
            ...
            public object newInstance(object[] objarr)
            {
                ...
                try
                {
                    return Activator.CreateInstance(type.AsReflection());
                }
        ...
        private abstract class FieldAccessorImplBase : global::sun.reflect.FieldAccessor, IReflectionException
        {
            ...
            private abstract class FieldAccessor<T> : FieldAccessorImplBase
            {
                ...
                private T lazyGet(object obj)
                {
                    ...
                    else if (fw.DeclaringType.IsRemapped && !fw.DeclaringType.TypeAsBaseType.AsReflection().IsInstanceOfType(obj))
                    {
            ...
            private Delegate GenerateFastGetter(Type delegateType, Type fieldType, RuntimeJavaField fw)
            {
                ...
                var dm = DynamicMethodUtil.Create("__<Getter>", fw.DeclaringType.TypeAsBaseType.AsReflection(), !fw.IsPublic || !fw.DeclaringType.IsPublic, fieldType, [typeof(IReflectionException), typeof(object), typeof(object)]);
                var il = JVM.Context.CodeEmitterFactory.Create(dm);
                if (fw.IsStatic)
                {
                    fw.EmitGet(il);
            ...
            private Delegate GenerateFastSetter(Type delegateType, Type fieldType, RuntimeJavaField fw)
            {
                ...
                var dm = DynamicMethodUtil.Create("__<Setter>", fw.DeclaringType.TypeAsBaseType.AsReflection(), !fw.IsPublic || !fw.DeclaringType.IsPublic, null, [typeof(IReflectionException), typeof(object), fieldType, typeof(object)]);
                var il = JVM.Context.CodeEmitterFactory.Create(dm);
                if (fw.IsStatic)
                {
