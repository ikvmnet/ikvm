        ...
        protected override void EmitUnsafeGetImpl(CodeEmitter il)
        {
            ...
            if (IsStatic)
            {
                if (IsFinal)
                {
                    ...
                    il.Emit(OpCodes.Call, typeof(RuntimeSimpleJavaField).GetMethod(nameof(UnsafeGetImplByRefNoInline), BindingFlags.Static | BindingFlags.NonPublic).MakeGenericMethod(fi.FieldType));
                }
                else
                {
                    il.Emit(OpCodes.Ldsfld, fi);
