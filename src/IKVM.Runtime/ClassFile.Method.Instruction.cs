                ...
                internal void Read(IKVM.ByteCode.Decoding.Instruction instruction, ClassFile classFile)
                {
                    ...
                    switch (instruction.OpCode)
                    {
                        ...
                        case IKVM.ByteCode.OpCode.LookupSwitch:
                            ...
                            if (_lookupswitch.Matches.Count < 0 || _lookupswitch.Matches.Count > 16384)
                                throw new ClassFormatError("Incorrect lookupswitch");

                            arg1 = _lookupswitch.DefaultTarget;

                            var lookupswitchentries = new SwitchEntry[_lookupswitch.Matches.Count];
                            for (int i = 0; i < _lookupswitch.Matches.Count; i++)
                            {
                                lookupswitchentries[i].value = _lookupswitch.Matches[i].Key;
                                lookupswitchentries[i].target = _lookupswitch.Matches[i].Target;
                            }
                        ...
                        case IKVM.ByteCode.OpCode.TableSwitch:
                            ...
                            var tableswitchentries = new SwitchEntry[_tableswitch.High - _tableswitch.Low + 1];
                            for (int i = _tableswitch.Low; i < _tableswitch.High; i++)
                            {
                                tableswitchentries[i].value = i;
                                tableswitchentries[i].target = _tableswitch.Matches[i];
                            }

                            // do the last entry outside the loop, to avoid overflowing "i", if high == int.MaxValue
                            tableswitchentries[_tableswitch.High - _tableswitch.Low].value = _tableswitch.High;
                            tableswitchentries[_tableswitch.High - _tableswitch.Low].target = _tableswitch.Matches[_tableswitch.High];

                            switch_entries = tableswitchentries;
                            break;
                        default:
                            throw new ClassFormatError("Invalid opcode: {0}", instruction.OpCode);
                    }
