...
namespace IKVM.CoreLib.Symbols
{
    ...
    internal ref partial struct AssemblyIdentityParser
    {
        ...
        public readonly struct AssemblyIdentityParts
        {

            public AssemblyIdentityParts(string name, Version? version, string? cultureName, AssemblyNameFlags flags, byte[]? publicKeyOrToken)
            {
                _name = name;
                _version = version;
        ...
        private AssemblyIdentityParser(ReadOnlySpan<char> input)
        {
            if (input.Length == 0)
        ...
        internal static bool TryParse(ReadOnlySpan<char> name, ref AssemblyIdentityParts parts)
        {
        ...
        private bool TryParse(ref AssemblyIdentityParts result)
        {
            ...
            result = new AssemblyIdentityParts(name, version, cultureName, flags, pkt);
            return true;
        }
