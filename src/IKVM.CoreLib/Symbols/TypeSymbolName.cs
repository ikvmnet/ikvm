...
namespace IKVM.CoreLib.Symbols
{
    ...
    sealed class TypeSymbolName
    {
        ...
        private readonly TypeSymbolName? _elementOrGenericType;
        private readonly TypeSymbolName? _declaringType;
        private readonly ImmutableArray<TypeSymbolName> _genericArguments;
        private string? _name, _fullName, _assemblyQualifiedName;

        internal TypeSymbolName(string? fullName,
            AssemblyIdentity? assemblyName,
            TypeSymbolName? elementOrGenericType = default,
            TypeSymbolName? declaringType = default,
            ImmutableArray<TypeSymbolName>.Builder? genericTypeArguments = default,
            ...
            int nestedNameLength = -1)
        {
            ...
            _genericArguments = genericTypeArguments is null
                ? ImmutableArray<TypeSymbolName>.Empty
        ...
        /// <summary>
        /// Returns assembly name which contains this type, or null if this <see cref="TypeSymbolName"/> was not
        ...
        public TypeSymbolName DeclaringType
        {
            get
            {
        ...
        public string FullName
        {
            get
            {
                ...
                if (_fullName is null)
                {
                    ...
                    {
                        _fullName = TypeSymbolNameParserHelpers.GetGenericTypeFullName(GetGenericTypeDefinition().FullName.AsSpan(), _genericArguments.AsSpan());
                    }
                    else if (IsArray || IsPointer || IsByRef)
                    {
                        ...
                        _fullName = TypeSymbolNameParserHelpers.GetRankOrModifierStringRepresentation(_rankOrModifier, ref builder);
                    }
                    else
                    {
        ...
        /// rank or its bounds.
        /// </summary>
        public bool IsArray => _rankOrModifier == TypeSymbolNameParserHelpers.SZArray || _rankOrModifier > 0;
        ...
        /// </summary>
        public bool IsByRef => _rankOrModifier == TypeSymbolNameParserHelpers.ByRef;
        ...
        /// </summary>
        public bool IsSZArray => _rankOrModifier == TypeSymbolNameParserHelpers.SZArray;
        ...
        /// </summary>
        public bool IsPointer => _rankOrModifier == TypeSymbolNameParserHelpers.Pointer;
        ...
        public string Name
        {
            get
            {
                ...
                if (_name is null)
                {
                    if (IsConstructedGenericType)
                    {
                        _name = TypeSymbolNameParserHelpers.GetName(GetGenericTypeDefinition().FullName.AsSpan()).ToString();
                    }
                    else if (IsPointer || IsByRef || IsArray)
                    {
                        ValueStringBuilder builder = new(stackalloc char[64]);
                        builder.Append(GetElementType().Name);
                        _name = TypeSymbolNameParserHelpers.GetRankOrModifierStringRepresentation(_rankOrModifier, ref builder);
                    }
                    else if (_nestedNameLength > 0 && _fullName is not null)
                    {
                        _name = TypeSymbolNameParserHelpers.GetName(_fullName.AsSpan(0, _nestedNameLength)).ToString();
                    }
                    else
                    {
                        _name = TypeSymbolNameParserHelpers.GetName(FullName.AsSpan()).ToString();
                    }
                }
        ...
        /// <summary>
        /// Represents the total number of <see cref="TypeSymbolName"/> instances that are used to describe
        ...
        /// as the total number of <see cref="TypeSymbolName"/> instances that would be created if
        /// you were to totally deconstruct this instance and visit each intermediate <see cref="TypeSymbolName"/>
        ...
        /// involves inspecting 8 <see cref="TypeSymbolName"/> instances total:
        /// <list type="bullet">
        ...
        public int GetNodeCount()
        {
            ...
            else if (IsConstructedGenericType)
            {
                ...
                foreach (TypeSymbolName genericArgument in GetGenericArguments())
                {
        ...
        public TypeSymbolName GetElementType()
        {
            if (!(IsArray || IsPointer || IsByRef))
            {
                TypeSymbolNameParserHelpers.ThrowInvalidOperation_NoElement();
            }
        ...
        public TypeSymbolName GetGenericTypeDefinition()
        {
            if (!IsConstructedGenericType)
            {
                TypeSymbolNameParserHelpers.ThrowInvalidOperation_NotGenericType();
            }
        ...
        public static TypeSymbolName Parse(ReadOnlySpan<char> typeName)
            => TypeNameParser.Parse(typeName, throwOnError: true)!;
        ...
        /// <summary>
        /// If this <see cref="TypeSymbolName"/> represents a constructed generic type, returns an array
...
#else
        ImmutableArray<TypeSymbolName> GetGenericArguments() => _genericArguments;
#endif
