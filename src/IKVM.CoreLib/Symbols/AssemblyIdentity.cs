...
namespace IKVM.CoreLib.Symbols
{
    ...
    public class AssemblyIdentity
    {
        ...
        {
            AssemblyIdentityParser.Parts parts = default;
            if (!assemblyName.IsEmpty && AssemblyIdentityParser.TryParse(assemblyName, ref parts))
            {
                result = new(parts.Name, parts.Version, parts.CultureName, parts.PublicKeyOrToken?.ToImmutableArray() ?? [], parts.PublicKeyOrToken != null);
                return true;
            }
...
#pragma warning restore SYSLIB0037 // Type or member is obsolete
        ...
        /// <summary>
        /// Initializes a new instance of the <seealso cref="AssemblyIdentity"/> class based on the passed <see cref="AssemblyName"/>.
        /// </summary>
        ...
        public static AssemblyIdentity FromAssemblyName(AssemblyName name)
        {
            return new AssemblyIdentity(
                ...
                name.Version,
                name.CultureName,
                name.GetPublicKey()?.ToImmutableArray() ?? name.GetPublicKeyToken()?.ToImmutableArray() ?? default,
                name.GetPublicKey() != null,
                name.ContentType,
#pragma warning disable SYSLIB0037 // Type or member is obsolete
