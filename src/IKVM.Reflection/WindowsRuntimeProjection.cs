...
namespace IKVM.Reflection
{
    ...
    sealed class WindowsRuntimeProjection
    {
        ...
        static readonly Dictionary<TypeName, Mapping> projections = new Dictionary<TypeName, Mapping>();

        readonly ModuleReader module;
        readonly Dictionary<int, string> strings;
        readonly Dictionary<string, int> added = new Dictionary<string, int>();
        readonly int[] assemblyRefTokens = new int[(int)ProjectionAssembly.Count];
        int typeofSystemAttribute = -1;
        int typeofSystemAttributeUsageAttribute = -1;
        int typeofSystemEnum = -1;
        int typeofSystemIDisposable = -1;
        int typeofSystemMulticastDelegate = -1;
        int typeofWindowsFoundationMetadataAllowMultipleAttribute = -1;
        bool[] projectedTypeRefs;
        ...
        WindowsRuntimeProjection(ModuleReader module, Dictionary<int, string> strings)
        {
            this.module = module;
        ...
        internal static void Patch(ModuleReader module, Dictionary<int, string> strings, ref string imageRuntimeVersion, ref byte[] blobHeap)
        {
            ...
            {
                var mscorlib = module.universe.CoreLib;
                imageRuntimeVersion = mscorlib.__IsMissing ? "v4.0.30319" : mscorlib.ImageRuntimeVersion;
            }

            var obj = new WindowsRuntimeProjection(module, strings);
        ...
        void PatchAssemblyRef(ref byte[] blobHeap)
        {
        ...
        void PatchTypeRef()
        {
            ...
            for (int i = 0; i < typeRefs.Length; i++)
            {
                ...
                if (projections.TryGetValue(typeName, out var mapping) && mapping != null)
                {
        ...
        void PatchCustomAttribute(ref byte[] blobHeap)
        {
            var memberRefs = module.MemberRef.records;
            ...
            {
                if (memberRefs[i].Class == typeofSystemAttributeUsageAttribute && module.GetString(memberRefs[i].Name) == ".ctor")
                    ctorSystemAttributeUsageAttribute = (MemberRefTable.Index << 24) + i + 1;
                else if (memberRefs[i].Class == typeofWindowsFoundationMetadataAllowMultipleAttribute && module.GetString(memberRefs[i].Name) == ".ctor")
            ...
            if (ctorSystemAttributeUsageAttribute != -1)
            {
                ...
                for (int i = 0; i < customAttributes.Length; i++)
                {
                    ...
                    {
                        var br = module.GetBlob(customAttributes[i].Value);
                        br.ReadInt16();

                        var targets = MapAttributeTargets(br.ReadInt32());
        ...
        int AddAssemblyReference(string name, int publicKeyToken)
        {
        ...
        TypeName GetTypeRefName(int index)
        {
        ...
        TypeName GetTypeDefName(int index)
        {
        ...
        {
            if (!added.TryGetValue(str, out var index))
            {
                index = -(added.Count + 1);
                added.Add(str, index);
        ...
        Version GetMscorlibVersion()
        {
            var mscorlib = module.universe.CoreLib;
            return mscorlib.__IsMissing ? new Version(4, 0, 0, 0) : mscorlib.GetName().Version;
        }

        static bool HasAllowMultipleAttribute(CustomAttributeTable.Record[] customAttributes, int i, int ctorWindowsFoundationMetadataAllowMultipleAttribute)
        {
        ...
        static int GetAttributeUsageAttributeBlob(ref byte[] blobHeap, Dictionary<int, int> map, AttributeTargets targets, bool allowMultiple)
        {
            var key = (int)targets;
            if (allowMultiple)
                key |= unchecked((int)0x80000000);

            if (map.TryGetValue(key, out var blob) == false)
                map.Add(key, blob = AddBlob(ref blobHeap, new byte[] {
                    0x01, 0x00,
                    (byte)targets,
                    (byte)((int)targets >> 8),
                    (byte)((int)targets >> 16),
                    (byte)((int)targets >> 24),
                    0x01, 0x00, 0x54, 0x02, 0x0D, 0x41, 0x6C, 0x6C, 0x6F, 0x77, 0x4D, 0x75, 0x6C, 0x74, 0x69, 0x70, 0x6C, 0x65,
                    allowMultiple ? (byte)0x01 : (byte)0x00 }));

            return blob;
        }

        static int ReadTypeSpec(ByteReader br)
        {
            ...
            {
                0 => (TypeDefTable.Index << 24) + (encoded >> 2),
                1 => (TypeRefTable.Index << 24) + (encoded >> 2),
                2 => (TypeSpecTable.Index << 24) + (encoded >> 2),
                _ => throw new BadImageFormatException(),
            };
        }

        static int AddBlob(ref byte[] blobHeap, byte[] blob)
        {
            if (blob.Length > 127)
                throw new NotImplementedException();
